...
$(document).ready(function() {
	...
	if ( $('.index.buildings').length || $('.index.floors').length ) {
		...
		$('.index.buildings .nav .element').bind('click', function() {
			...
			if ( id != ooo) {
				...
				$('.bg').filter('.bg'+id).siblings('.bg').css({'top': -wh+'px'});
				$('.bg').filter('.bg'+id).siblings('.bg').stop(true, true).animate({'top': wh+'px'}, speed);
				$('.bg.bg'+id).stop(true, true).animate({'top': -wh+'px'}, speed);;
	...
	if ( $('.index').length ) {
		...
		var scrollobjectheight = $('.index .nav div#6').position().top-20;
		
		$('.index .nav > div').height(scrollobjectheight);
		var scrollobjectpreview = $('.index .nav > div').jScrollPane({
		...
		var current = 1;
		$('.index .nav > .prev').bind('click', function() {
			if ( current > 1 ) {
				if (current == 2) {
					current = current - 1;
					target = 0;
					...
					current = current - 1;
					var target = $('#'+current).position().top;
		...
		$('.index .nav > .next').bind('click', function() {
			var rp = $('.jspPane').height() - $('#'+current).position().top - $('.jspScrollable').height();
			if ( rp > 0 ) {
				current = current + 1;
				var target = $('#'+current).position().top;
				api.scrollToY(target, true);
...
*/
		...
		$('.index.buildings .object').each(function() {
			var modalheight = $(this).height()-20;
			$(this).find('.modal').height(modalheight);
			$(this).find('.links li a').bind('click', function() {
				var target = $(this).attr('href');
				//if ( target == 'map' ) {
				//	var newurl = $(this).parents('.object').find('.modal.map script').attr('src').replace(/height=\d+/, 'height='+600);
